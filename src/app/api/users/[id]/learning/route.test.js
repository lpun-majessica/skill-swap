...
describe("/api/users/[id]/learning", async () => {
  const targetUser = users[0];
  const newSkill = skills[0]._id;
  const existingSkill = targetUser.skillsToLearn[0];
  ...
  describe("DELETE", async () => {
    it("delete existing skill", async () => {
      ...
      const updatedSkills = body.skillsToLearn.map((skill) => skill.id);

      expect(updatedSkills).toHaveLength(targetUser.skillsToLearn.length - 1);
    ...
    it("delete non-existing skill", async () => {
      ...
      expect(updatedSkills).toHaveLength(targetUser.skillsToLearn.length);
  ...
  describe("POST", async () => {
    ...
    it("add existing skill", async () => {
      ...
      expect(response.status).toBe(201);
      expect(updatedSkills).toHaveLength(targetUser.skillsToLearn.length);
      expect(updatedSkills).toEqual(targetUser.skillsToLearn);
    });
  });
});
